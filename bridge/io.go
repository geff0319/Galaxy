...
func (a *App) OpenDirectoryDialog() FlagResult {
	folder, err := runtime.OpenDirectoryDialog(a.Ctx, runtime.OpenDialogOptions{
		Title:                "请选择下载目录",
		CanCreateDirectories: true,
	})
	if err != nil {
		gefflog.Err("打开选择目录对话框失败：" + err.Error())
		return FlagResult{false, "操作失败"}
	}
	fmt.Println("选择目录：" + folder)
	return FlagResult{true, folder}
}
