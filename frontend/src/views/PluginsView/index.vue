...
import PluginHubPersonal from './components/PluginHubPersonal.vue'
...
const showPluginHubPersonal = ref(false)
...
const handleImportPersonalPlugin = async () => {
  showPluginHubPersonal.value = true
}
...
const generateMenus = (p: PluginType) => {
 ...
 console.log(Object.keys(p.menus).length)
...
<template>
  ...
  <div v-else class="grid-list-header">
    ...
    <div  class="ml-auto">
      ...
      <Button @click="handleImportPersonalPlugin" type="link">
        {{ t('plugins.personalHub') }}
      </Button>
    </div>
    <Button
  ...
  <Modal
    v-model:open="showPluginHubPersonal"
    title="plugins.personalHub"
    :submit="false"
    mask-closable
    cancel-text="common.close"
    height="90"
